...
@Ankit.on(events.NewMessage(pattern="^[!?/]id"))
async def id(event):
...
@Ankit.on(events.NewMessage(pattern="^[!?/]info ?(.*)"))
async def info(event):

    sed = await Ankit(P(user_id=event.sender_id, offset=42, max_id=0, limit=80))
    hn = await Ankit(GetFullUserRequest(event.sender_id))
    text = "**✘ UserInfo:**\n\n"
    text += "**» Fɪʀsᴛ Nᴀᴍᴇ:** {}\n"
    text += "**» Lᴀsᴛ Nᴀᴍᴇ:** {}\n"
    text += "**» Usᴇʀ-ID:** `{}`\n"
    ...
    if not input_str:
          await Ankit.send_message(event.chat_id, text.format(hn.user.first_name, hn.user.last_name, event.sender_id, event.sender.username, sed.count, hn.about, event.sender_id))
          return
 
    input_str = event.pattern_match.group(1)
    ha = await Ankit.get_entity(input_str)
    hu = await Ankit(GetFullUserRequest(id=input_str))
    sedd = await Ankit(P(user_id=input_str, offset=42, max_id=0, limit=80))
...
@Ankit.on(events.callbackquery.CallbackQuery(data="misc"))
async def _(event):
