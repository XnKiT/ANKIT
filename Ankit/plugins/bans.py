...
@Ankit.on(events.NewMessage(pattern="^[!?/]kick ?(.*)"))
@is_admin
async def kick(event, perm):
    ...
    info = await Ankit.get_entity(us)
    await Ankit.kick_participant(event.chat_id, input_str or replied_user)
    await event.reply(f"Succesfully Kicked [{info.first_name}](tg://user?id={replied_user}) from {event.chat.title}")

@Ankit.on(events.NewMessage(pattern="^[!?/]kickme"))
async def kickme(event):
    ...
    check = await Ankit.get_permissions(event.chat_id, event.sender_id)
    if check.is_admin:
        await event.reply("Sorry but I can't kick admins!")
        return

    await event.reply("Ok, as your wish")
    await Ankit.kick_participant(event.chat_id, event.sender_id)

@Ankit.on(events.NewMessage(pattern="^[!?/]ban ?(.*)"))
@is_admin
async def ban(event, perm):
    ...
    us = msg.sender.username
    info = await Ankit.get_entity(us)
    await Ankit(EditBannedRequest(event.chat_id, replied_user, ChatBannedRights(until_date=None, view_messages=True)))
    await event.reply(f"Succesfully Banned [{info.first_name}](tg://user?id={replied_user}) in {event.chat.title}")

@Ankit.on(events.NewMessage(pattern="^[!?/]unban ?(.*)"))
@is_admin
async def unban(event, perm):
    ...
    us = msg.sender.username
    info = await Ankit.get_entity(us)
    await Ankit(EditBannedRequest(event.chat_id, replied_user, ChatBannedRights(until_date=None, view_messages=False)))
    await event.reply(f"Succesfully Unbanned [{info.first_name}](tg://user?id={replied_user}) in {event.chat.title}")

@Ankit.on(events.NewMessage(pattern="^[!?/]skick"))
@is_admin
async def skick(event, perm):
    ...
    await event.delete()
    await Ankit.kick_participant(event.chat_id, x)
    await event.reply(f"Succesfully Kicked [{info.first_name}](tg://user?id={replied_user}) from {event.chat.title}")

@Ankit.on(events.NewMessage(pattern="^[!?/]dkick"))
@is_admin
async def dkick(event, perm):
    ...
    await x.delete()
    await Ankit.kick_participant(event.chat_id, x.sender_id)
    await event.reply(f"Succesfully Kicked [{info.first_name}](tg://user?id={replied_user}) from {event.chat.title}")

@Ankit.on(events.NewMessage(pattern="^[!?/]dban"))
@is_admin
async def dban(event, perm):
    ...
    zx = (await event.get_reply_message())
    await zx.delete()
    await Ankit(EditBannedRequest(event.chat_id, x, ChatBannedRights(until_date=None, view_messages=True)))
...
@Ankit.on(events.NewMessage(pattern="^[!?/]sban"))
@is_admin
...
@Ankit.on(events.callbackquery.CallbackQuery(data="bans"))
