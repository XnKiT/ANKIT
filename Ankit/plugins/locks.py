...
@Ankit.on(events.NewMessage(pattern="^[!?/]lock ?(.*)"))
@is_admin
async def lock(event, perm):
    ...
    if "text" in input_str:
       ...
       await Ankit.edit_permissions(event.chat_id,
          send_messages=False, 
          send_media=False,
          send_stickers=False,
          send_gifs=False,
...
@Ankit.on(events.NewMessage(pattern="^[!?/]unlock ?(.*)"))
@is_admin
...
@Ankit.on(events.NewMessage(pattern="^[!?/]locktypes"))
...
@Ankit.on(events.callbackquery.CallbackQuery(data="locks"))
async def _(event):
